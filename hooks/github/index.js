...
function findBlockquoteClose(tokens, index) {
    ...
    do {
        ...
        if (type === "blockquote_close") depth--;
    } while (depth && index < tokens.length);
    return index - 1;
}
...
function transform(markdownIt, tokens, env) {
    ...
    for (let index = 0; index < tokens.length; index++) {
        ...
        while (inlineIndex < closeIndex && tokens[inlineIndex].type !== "inline") inlineIndex++;
        const inline = tokens[inlineIndex];
        const match = inline?.content.match(ALERT_MARKER);
        if (!match) continue;
        ...
        result[inlineIndex] = cloneToken(inline, { content, children });
        result[closeIndex] = cloneToken(tokens[closeIndex], {
            type: "markdown_preview_themes_alert_close",
            tag: "div",
        });
        index = closeIndex;
    }
